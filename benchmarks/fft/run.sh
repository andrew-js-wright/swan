...
bash -c "time numactl --physcpubind=+${CPUS[$cores]} --membind=0-$[(NUM_THREADS+7)/8-1] ./src_$config/$bench -n 20000000" > output_${bench}_${config}/${bench}_${config}_${cores}_${i}.txt 2>&1
done
done
